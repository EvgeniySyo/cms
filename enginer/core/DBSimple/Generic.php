...
define('DBSIMPLE_PARENT_KEY', 'PARENT_KEY'); // forest-based resultset support
define('DBSIMPLE_CAST', 'CAST');             // cast support
...
class DbSimple_Generic
{
    ...
    static function& connect($dsn)
    {
		...
		$schema = ucfirst($parsed['scheme']);
        $class = 'DbSimple_'.ucfirst($schema);
...
{
	// Языковые таблицы
	private $content_tables = null;
	private $content_table_prefix = null;
	private $content_tables_replace = null;

	public function setContentTables($prefix, $tables)
	{
		$this->content_tables = $tables;
		$this->content_table_prefix = $prefix;
		$this->content_tables_replace = null;
	}

	public function getContentTablesPrefix()
	{
		return $this->content_table_prefix;
	}

	private function contentReplaceQuery($query)
	{
		if (!$this->content_table_prefix || !$this->content_tables) return $query;
		if (!$this->content_tables_replace)
		{
			foreach ($this->content_tables as $table)
				$this->content_tables_replace[$table] = $this->_identPrefix.$this->content_table_prefix.substr($table, 2);
		}
		return str_replace(array_keys($this->content_tables_replace), array_values($this->content_tables_replace), $query);
	}
	 ...
	 * Заключает строку в кавычки базы данных, корректно экранируя
	 * специальные символы. Если $isIdent=true, значение обкавычивается как идентификатор
	 * (напр: `value` в MySQL, "value" в Firebird, [value] в MSSQL).
	 */
	 ...
	 */
	public function setLogger($logger, $db_id)
	{
		$prev = $this->_logger;
		$this->_logger = $logger;
		$this->_db_id = $db_id;
	...
	private function _query($query, &$total)
	{
		...
		{
			$this->_logQuery($query);
			$q_test = substr($query[0], 0, 2);
			if (in_array($q_test, array('D ', 'E ')))
			{
				$query[0] = substr($query[0], 2);
				$this->_expandPlaceholders($query, false);
                $query[0] = preg_replace("#\s+#uis", " ", $query[0]);
				if ($q_test === 'D ')
					die($query[0]);
				else
					echo $query[0]."\n";
			...
			$queryTime = microtime(true) - $qStart;
            // echo "![Query time: ".round($queryTime, 1)."]".$query[0]."\n";
			...
			if (!empty($this->attributes['CAST']) && is_array($result)) {
				foreach ($result as &$current_row) {
					if (is_array($current_row)) {
						$key = key($current_row);
						if ($key) {
							$current_row[$key] = $this->castField($current_row[$key], $this->attributes['CAST']);
						}
					}
				}
			}
	...
	private function _expandPlaceholdersFlow($query)
	{
		...
		$query = preg_replace_callback(
			...
			$this->contentReplaceQuery($query)
		);
		return $query;
	}
	 ...
	 * @return float|int
	 */
	private function castField($value, $type) {
	...
	private function _logQueryStat($queryTime, $fetchTime, $firstFetchTime, $rows)
	{
		...
		$this->_statistics['qtime'] = $queryTime;
...
abstract class DbSimple_Generic_LastError
{
	...
	protected function _setLastError($code, $msg, $query)
	{
		...
		$show_stack = 1; //$this->showStack;
		if ($t = $this->findLibraryCaller($show_stack))
		{
			if (!$show_stack)
	...
	private function debug_backtrace_smart($ignoresRe=null, $returnCaller=false)
	{
		...
		for ($i=0, $n=count($trace); $i<$n; $i++) {
			...
			if (isset($t['over_function']) && ($t['over_function'] == "call_user_func_array")) continue;
			// On each iteration we consider ability to add PREVIOUS frame
			// to $smart stack.
			if ($returnCaller && !empty($t['class']) && ($t['class'] == 'DbSimple_Generic_Database')) continue;
